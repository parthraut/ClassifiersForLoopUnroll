...
static void tests()
{
    ...
    int size = 1000000;
    char longString[size];
    for (int i = 0; i < size; i++) {
        int randomIndex = rand() % (sizeof(characters) - 1);
        longString[i] = characters[randomIndex];
    }

    test_string(longString, "&3ddy2", 7, 11);
    test_string(longString, "DNC}=jHS2zN!7;E", 67, 67);
    test_string(longString, "840,($ {ws", 91, 88);
    test_string(longString, "JDfa*we?z&bL", 77, 76);
    test_string(longString, "r'qC0$sss;Ahf", 8, 90);
    test_string(longString,
